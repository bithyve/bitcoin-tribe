...
- (NSString *)createErrorResponse:(NSString *)errorMessage {
    NSDictionary *errorDict = @{@"error": errorMessage ?: @"Unknown error"};
...
RCT_EXPORT_METHOD(generateKeys:(NSString*)network
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    @try {
      ...
      NSString *errorResponse = [self createErrorResponse:exception.reason];
      [self resolvePromise:resolve withResult:errorResponse];
    }
  });
}

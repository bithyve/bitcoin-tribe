...
export default class ElectrumClient {
  ...
  public static async reconnect() {
    ...
    if (
      ELECTRUM_CLIENT.connectionAttempt >=
      ELECTRUM_CLIENT_CONFIG.maxConnectionAttempt
    ) {
  ...
  public static forceDisconnect() {
    if (!ELECTRUM_CLIENT.electrumClient) {
  ...
  public static getBlockchainHeaders = async (): Promise<{
    height: number;
    hex: string;
  ...
  public static getNextDefaultPeer() {
    ...
    if (!peers || ELECTRUM_CLIENT.currentPeerIndex > peers.length - 1) {
      return null;
  ...
  public static setActivePeer(
    ...
    if (config.NETWORK_TYPE === NetworkType.TESTNET) {
      ELECTRUM_CLIENT_CONFIG.predefinedTestnetPeers =
        shufflePeers(defaultNodes);
    } else {
      ELECTRUM_CLIENT_CONFIG.predefinedPeers = shufflePeers(defaultNodes);
    }

    // set active node
    let activeNode =
      currentPeerToUse || privateNodes.filter(node => node.isConnected)[0];
    if (!activeNode && defaultNodes.length) {
      activeNode = ElectrumClient.getNextDefaultPeer();
    } // pick one of the default nodes
  ...
  public static async syncUTXOByAddress(
    ...
    for (let itr = 0; itr < chunks.length; itr += 1) {
      ...
      const results =
        await ELECTRUM_CLIENT.electrumClient.blockchainScripthash_listunspentBatch(
          scripthashes,
        );
  ...
  public static async syncHistoryByAddress(
    ...
    for (let itr = 0; itr < chunks.length; itr += 1) {
      ...
      const results =
        await ELECTRUM_CLIENT.electrumClient.blockchainScripthash_getHistoryBatch(
          scripthashes,
        );
  ...
  public static async getTransactionsById(
    ...
    for (const chunk of chunks) {
      ...
      results =
        await ELECTRUM_CLIENT.electrumClient.blockchainTransaction_getBatch(
          chunk,
          verbose,
        );
  ...
  public static async estimateFee(numberOfBlocks: number = 1) {
    ...
    const feePerKB = await ELECTRUM_CLIENT.electrumClient.blockchainEstimatefee(
      numberOfBlocks,
    ); // in bitcoin
    if (feePerKB === -1) {
      return 1;
  ...
  public static async broadcast(txHex: string) {
    ...
    return ELECTRUM_CLIENT.electrumClient.blockchainTransaction_broadcast(
      txHex,
    );
  }

...
export class ChatKeyManagerError extends Error {
  constructor(message: string, public readonly code?: string) {
    super(message);
    this.name = 'ChatKeyManagerError';
  }
}

export class KeyExchangeError extends ChatKeyManagerError {
...
export class EncryptionError extends ChatKeyManagerError {
  constructor(message: string) {
...
export class DecryptionError extends ChatKeyManagerError {
  constructor(message: string) {
...
/**
 * Stateless Chat Key Manager for Ed25519 Keys
 * Pure utility class with static methods for key exchange and AES encryption
 * No instance state - all methods accept required parameters directly
 * 
 * @example
 * ```typescript
 * // All methods are static - no instances needed
 * const sharedSecret = ChatKeyManager.performKeyExchange(myKeyPair, peerPublicKey);
 * const sessionKeys = ChatKeyManager.generateSessionKeys();
 * const encrypted = ChatKeyManager.encryptMessage(message, sessionKeys.aesKey);
 * const decrypted = ChatKeyManager.decryptMessage(encrypted, sessionKeys.aesKey);
 * ```
 */
export class ChatKeyManager {
  ...
  static performKeyExchange(keyPair: KeyPair, peerPublicKeyHex: string): string {
    try {
      const publicKey = ChatKeyManager.hexToUint8Array(keyPair.publicKey);
      const privateKeyHex = keyPair.secretKey.length === 128 
        ? keyPair.secretKey.slice(0, 64) 
        : keyPair.secretKey;
      const privateKey = ChatKeyManager.hexToUint8Array(privateKeyHex);
      ...
      const peerPublicKey = ChatKeyManager.hexToUint8Array(peerPublicKeyHex);
      ...
      const myPubHex = ChatKeyManager.uint8ArrayToHex(publicKey);
      const peerPubHex = ChatKeyManager.uint8ArrayToHex(peerPublicKey);
      ...
      const secretHex = ChatKeyManager.uint8ArrayToHex(sharedSecret);
      const info = 'ChatKeyManager-KeyExchange-v1';
      ...
      throw new KeyExchangeError(`Key exchange failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static encryptKeys(aesKeyHex: string, sharedSecretB64: string): EncryptedKeys {
    try {
      ...
      throw new EncryptionError(`Key encryption failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static decryptKeys(encryptedKeyData: EncryptedKeys, sharedSecretB64: string): SessionKeys {
    try {
      ...
      throw new DecryptionError(`Key decryption failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static encryptMessage(message: string, aesKeyHex: string): EncryptedMessage {
    try {
      ...
      throw new EncryptionError(`Message encryption failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  static decryptMessage(encryptedData: EncryptedMessage, aesKeyHex: string): string {
    try {
      ...
      throw new DecryptionError(`Message decryption failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

...
function ChangePinContainer() {
  ...
  const changePin = useMutation(ApiHandler.changePin);
  const { key } = useContext(AppContext);

  useEffect(() => {
    if (changePin.error) {
      Toast(onBoarding.errorSettingPin, true);
    } else if (changePin.isSuccess) {
      Toast('Passcode changed successfully', false);
      navigation.goBack();
    }
  }, [changePin.error, changePin.isSuccess, changePin.data]);

  const disbleProceed = useMemo(() => {
    if (passcode === '' && confirmPasscode === '') {
      return false;
    } else if (passcode.length > 0 || confirmPasscode.length > 0) {
    ...
    <>
      ...
      <View style={styles.ctaWrapper}>
        <Buttons
          ...
          primaryOnPress={() => changePin.mutate({ key, pin: passcode })}

...
import { Asset, Coin, Collectible, UniqueDigitalAsset } from 'src/models/interfaces/RGBWallet';
...
function SelectAssetToSend() {
  ...
  const udas = useQuery<UniqueDigitalAsset[]>(RealmSchema.UniqueDigitalAsset).filtered('balance.spendable > 0');
  const assetsData: Asset[] = useMemo(() => {
    const combined: Asset[] = [...coins.toJSON(), ...collectibles.toJSON(), ...udas.toJSON()];

...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST display a Collectible's image, name, ticker, balance (spendable, settled, future), and recent transactions when the user opens the collectible's detail screen.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST display a UDA's full-screen image, name, asset ID, and transfer history when the user opens the UDA detail screen.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the user to view the complete transfer history for any asset, showing kind (send, receive, or issuance), amount, status, and timestamp per entry.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the user to send any Coin, IFA, or Collectible with a positive spendable balance to a valid RGB invoice.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the user to send a UDA with a spendable balance of at least one to a valid RGB invoice. The transfer amount is always one.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MAY offer a gas-free transfer option for Coin assets with six decimal places of precision.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the user to generate a Blinded or Witness RGB invoice for receiving any asset type, displaying it as a QR code with copy and share options.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the user to scan or paste an RGB invoice QR code to initiate a send.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST display full details of an individual transfer including amount, status, timestamp, and transaction ID.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MAY present a claim button on a Coin detail screen when the coin has an active campaign and the user is eligible.

#### Scenario: Successful claim
- GIVEN the coin has an active campaign and the user meets eligibility criteria
- WHEN the user taps the claim button
- THEN the system initiates a reward transfer

#### Scenario: Insufficient sats for campaign
- GIVEN the campaign requires a Bitcoin balance and the user has insufficient sats
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST display a collection's banner, icon, name, description, issued item count, total supply (or unlimited indicator), and a scrollable grid of its UDA thumbnails.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST provide a fullscreen paging gallery for a collection's UDAs, allowing the user to swipe between items and view details or perform actions for each.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the collection issuer to add new UDA items to an existing collection they issued.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the asset or collection issuer to verify their Twitter (X) identity via OAuth, and MUST reflect the verified status on the asset detail screen.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the asset or collection issuer to verify domain ownership by adding a DNS TXT record and confirming it through the app.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST allow the asset issuer to link a public tweet to the asset by providing a valid tweet URL.
...
WHEN appType is `NODE_CONNECT` or `SUPPORTED_RLN`, the system MUST block all send and receive actions while the node is initializing, and display a status notice to the user.
